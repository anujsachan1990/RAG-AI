...
function ComponentRenderer({ spec, onButtonClick }: ComponentRendererProps): ReactElement {
  ...
  switch (componentType.toLowerCase()) {
    ...
    case "grid":
      return (
        <div className={`grid gap-4 ${props.columns ? `grid-cols-${props.columns}` : "grid-cols-2"}`}>
          {renderChildren(children)}
        </div>
      )
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              {props.title && <div className="font-semibold text-foreground break-words">{props.title}</div>}
              {props.label && <div className="text-sm text-muted-foreground break-words">{props.label}</div>}
              {props.description && (
                <div className="mt-1 text-xs text-muted-foreground break-words">{props.description}</div>
              )}
            </div>
          </div>
        </Card>
      ...
      return (
        <div className={`relative overflow-hidden ${props.rounded ? "rounded-lg" : ""} ${props.className || ""}`}>
          <img
            ...
            className={`w-full h-auto object-cover ${props.size === "small" ? "max-w-xs" : props.size === "medium" ? "max-w-md" : props.size === "large" ? "max-w-2xl" : ""}`}
            loading="lazy"
          />
